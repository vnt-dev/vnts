...
impl<B: AsRef<[u8]>> IcmpPacket<B> {
    ...
    pub fn header_other(&self) -> HeaderOther {
        match self.kind() {
            Kind::EchoReply | Kind::EchoRequest | Kind::TimestampRequest |
            Kind::TimestampReply | Kind::InformationRequest | Kind::InformationReply => {
                let ide = (&self.buffer.as_ref()[4..]).read_u16::<BigEndian>().unwrap();
                let seq = (&self.buffer.as_ref()[6..]).read_u16::<BigEndian>().unwrap();
    ...
    pub fn description(&self) -> Description<&[u8]> {
        use std::io::Cursor;
        match self.kind() {
            Kind::DestinationUnreachable | Kind::TimeExceeded | Kind::ParameterProblem |
            Kind::SourceQuench | Kind::Redirect => {
                match IpV4Packet::new(self.payload()) {
                    Ok(d) => {
                        Description::Ip(d)
                    }
                    Err(_) => {
                        Description::Other(self.payload())
                    }
                }
            }
            Kind::TimestampRequest | Kind::TimestampReply => {
                let mut buffer = Cursor::new(self.payload());
                Description::Timestamp(buffer.read_u32::<BigEndian>().unwrap(),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Description::Ip(packet) => {
                f.debug_struct(&format!("{:?}",packet)).finish()
            }
            Description::Timestamp(originate, receive, transmit) => {
                f.debug_struct("")
                    .field("originate", originate)
                    .field("receive", receive)
                    .field("transmit", transmit).finish()
            }
            Description::Other(bytes) => {
                f.debug_struct(&format!("{:?}",bytes)).finish()
            }
        }
    }
}
