...
lazy_static::lazy_static! {
     static ref MAC_ADDRESS_SESSION:Cache<(String,String),()> = Cache::builder()
        .time_to_idle(Duration::from_secs(60*60*24*7)).eviction_listener(|k:Arc<(String,String)>,_,_|{
    ...
    static ref SESSION:Cache<SocketAddr,Context> = Cache::builder()
        .time_to_idle(Duration::from_secs(10)).eviction_listener(|_,context:Context,_|{
...
fn handle(udp: &UdpSocket, buf: &[u8], addr: SocketAddr) -> Result<()> {
    ...
    )
    {
        ...
        //todo 暂时写死地址 需要验证token,从数据库根据token读出网关
        ...
        if let Some(v) = VIRTUAL_NETWORK.optionally_get_with(request.token.clone(), || {
            ...
            for (mac_address, device_info) in &lock.virtual_ip_map {
    ...
    if let Some(context) = SESSION.get(&addr) {
        if DEVICE_ADDRESS
